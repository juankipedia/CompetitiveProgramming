...
lli get_at_least_x(int ss, int se, int si, lli x){
    if(st[si] < x) return n;
    if(ss == se) return ss;
    int mid = ss + (se - ss) / 2;
    if(x <= st[2 * si + 1]) return get_at_least_x(ss, mid, 2 * si + 1, x);
    else if(x <= st[2 * si + 2]) return get_at_least_x(mid + 1, se, 2 * si + 2, x);
    else return n;
}
...
int main(){
    ...
    while (m--){
        ...
        if(q == 1){
            ...
            cin >> val;
            int ans = get_at_least_x(0, n - 1, 0, val);
