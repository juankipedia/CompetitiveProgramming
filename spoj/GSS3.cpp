...
vector<lli> update(int ss, int se, int si, int i, lli val){
    if(i < ss || i > se) return {STP[si], STS[si], STM[si], STT[si]};
    if(ss == se){
        A[i] = val;
        STP[si] = STS[si] = STM[si] = STT[si] = A[i];
        return {STP[si], STS[si], STM[si], STT[si]};
    }

    int MID = get_mid(ss, se);
    vector<lli> L = update(ss, MID, 2 * si + 1, i, val);
    vector<lli> R = update(MID + 1, se, 2 * si + 2, i, val);
